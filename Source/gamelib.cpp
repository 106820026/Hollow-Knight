...
namespace game_framework {
	...
	void CGameState::ShowInitProgress(int percent)
	{
		...
		const int y1 = (SIZE_Y - bar_height) / 2 + 220;
		...
		CMovingBitmap begin;						// Hollow Knight的Loading背景圖
		CMovingBitmap loading;						// 貼上loading圖示
		begin.LoadBitmap(IDB_BEGIN);
		loading.LoadBitmap(IDB_LOADING, RGB(0, 0, 0));
		begin.SetTopLeft(0, 0);
		loading.SetTopLeft((SIZE_X - loading.Width()) / 2, y1 - 2 * loading.Height() + 20);
		begin.ShowBitmap();
		loading.ShowBitmap();
		//
		// 以下為CDC的用法
		...
		: NUM_GAME_STATES(9)
	{
		...
		gameStateTable[GAME_STATE_RUN1] = new CGameStateRun1(this);
		gameStateTable[GAME_STATE_RUN1_2] = new CGameStateRun1_2(this);
